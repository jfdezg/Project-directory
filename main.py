...
def get_vectorstore(chunks):
    if os.path.exists(VECTORSTORE_DIR):
        ...
        vstore.save_local(VECTORSTORE_DIR)
        return vstore
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("🗑️ Forzar reprocesado"):
            if os.path.exists(VECTORSTORE_DIR):
                shutil.rmtree(VECTORSTORE_DIR)
                st.success("🔄 Índice eliminado. Se generará uno nuevo al procesar.")
        ...
        if st.button("🚀 Process") and pdf_docs:
            with st.spinner("Procesando documentos..."):
                ...
                st.session_state.chat_history = []  # ← se limpia el historial aquí
